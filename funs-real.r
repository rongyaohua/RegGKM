...
# modi up.beta 20230206
# add if(any(is(NaN))|(sd(yy)==0)
#  for "stop("y is constant; gaussian glmnet fails at standardization step")"
...
up.beta<-function(Y,X,Z,alpha,delta,lambda_1,weig){
  ...
  if(any(is.na(yy))|(sd(yy)==0)){ # modi 20230206 for "stop("y is constant; gaussian glmnet fails at standardization step")"
  		betaa <- rep(0,ncol(X))
  }else if(ncol(X)==1){
...
up.delta.spg <- function(X,Z,u,TAU,R,len,alpha,betaa,delta,lambda_2,lambda_3){
	...
	# modi 20230205 add u to ... the parameters of f and fg
	...
	if ((max(fg_res) < 0) & (max(delta) ==0)){
		...
		spg_res <- spg(par=delta,fn=f,gr=fg,X=X,Z=Z,u=u,TAU=TAU,R=R,len=len,alpha=alpha,betaa=betaa,lambda_2=lambda_2,lambda_3=lambda_3,N=N,Q=Q,lower=rep(0,Q),upper=rep(1,Q),control=list(maximize=TRUE,checkGrad=FALSE,eps=1e-5,ftol=1e-9,trace=FALSE),quiet=TRUE) #modi 20220808 1e-4 
		delta_n <- spg_res$par
...
Qf <- function(X,Z,u,TAU,R,len,E,len_E,alpha_nn,beta_nn,delta_nn,lambda_1,lambda_2,lambda_3){
	...
	#	print(paste( "Qf iter = ",iter, "like_new = ", like_new, "num of non-zero delta_n = ",sum(delta_n != 0)))
		...
		print(c(like_new,(1/N) * (t(TAU)%*% (X %*% beta_n + hf(Z,alpha_n,delta_n) - A_n)), -lambda_1  * sum(abs(beta_n)), -lambda_2 * sum(delta_n), -1/2 *lambda_3 * t(alpha_n) %*% Kf(Z,delta_n) %*% alpha_n))
		...
		print(paste('like_diff',like_diff))
	...
	result<-list(mess=mess,like_new=like_new,iter=iter,betaa=beta_n,alpha=alpha_n,delta=delta_n)  #result of iter-1 iteration.  Why
	return(result)
}
##############
...
lamf<- function(X,Z,TAU,TIM,Xvalid,Zvalid,TAUvalid,TIMvalid,#Hvalid, modi 20220809 Hvalid delete in real data
                 lambda_1_seq,lambda_2_seq,lambda_3_seq){#,alpha_nn,beta_nn,delta_nn){
	...
	alpha_nn <- runif(N,0,1); alpha_nn <- alpha_nn/sum(alpha_nn) #rep(0.5,N)
